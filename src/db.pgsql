...
COMMENT ON VIEW internal.account_view IS 'Internal view to be able to more simply perform operations on the recursive account structure';

CREATE VIEW public.account(full_name, parent_name, name) AS
  SELECT full_name, parent_name, name FROM internal.account_view;
